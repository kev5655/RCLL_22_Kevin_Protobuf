...
import org.robocup_logistics.llsf_msgs.ZoneProtos.Zone;
...
//          rBLog.debug("Lichtmuster: " + lightPatternMap.toString());
    ...
    /**
     * Die in der Explophase entdeckten Maschinen (MPS) werden der Refbox gemeldet. Vom Objekt
     * Station kommen die ID, die Koordinaten und das Lampenmuster. Mit der Methode
     * fieldcommander.Zones.zoneDetector() werden die Koordinaten in die entsprechende Zone
     * umgewandelt. Mit der Methode fieldcommander.Station.getLamps wird das Lampenmuster als
     * Boolean geholt und in ein String mit der Form "000" umgewandelt. Erst wenn alle drei
     * Informationen vorhanden sind, wird ein neuer Maschinenreporteintrag generiert.
     *
     *
     * @see fieldcommander.Mps#idToNameRefbox(int)
     * @see fieldcommander.Zones#zoneDetector(drive.Coord)
     * @see fieldcommander.Mps#getLamps()
     * @see cam.Lamp
     * @param discovMachine
     * @param teamColor
     * @return MachineReport
     */
    public static MachineReport prepareMachineReport(Station discovMachine, Team teamColor)
    {
        MachineReport mr = null;
        double x;
        double y;

        String machineName = discovMachine.getStationName();
        Integer rotation = discovMachine.getAngle();  //entweder mit null oder anderes Attribut für Wahrscheinlichkeit oder Qualität
        String zoneTxt = discovMachine.getZone();

        try
        {
            // Die ID kommt aus dem vom Broker gesendeten Objekt.
            if (machineName != null && !zoneTxt.equals("0"))
            {
                Zone zone = Field.getRBZone(zoneTxt);
                MachineReportProtos.MachineReportEntry mi; //Eintrag für den Maschinenreport.
                if (discovMachine.getSendAngle())
                {
                    mi = MachineReportProtos.MachineReportEntry
                        .newBuilder()
                        .setName(machineName)
                        .setZone(zone)
                ...
                mr = MachineReport.newBuilder().addMachines(mi).setTeamColor(teamColor).build();
                rBLog.debug("Send machine to refbox. Name: " + machineName + "/Zone:" + zone + "/Rot:" + rotation);
            }
        } catch (Exception e)
        {
            rBLog.debug("Exception in prepareMachineReport" + e);
        }
