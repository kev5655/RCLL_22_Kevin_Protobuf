...
import comm.MqttCom;
import comm.TPC;
import dataobjects.ConsoleColors;
...
public class ComRefBox implements MqttCallback, Runnable
{
    ...
    private void setPeerPublic(boolean isMasterFlag)
    {
        ...
        {
            rBLog.fatal(ConsoleColors.coloredMsg(ConsoleColors.RED,"Error while starting public peer. Message: " + e.getMessage()));
            System.exit(0);
        }
    }
    ...
    public void setPeerPrivate(Team team, boolean enableBeacon)
    {
        ...
        if (peerPrivate != null)
        {
            ...
            {
                rBLog.fatal(ConsoleColors.coloredMsg(ConsoleColors.RED ,"Exception while start private peer. Message: " + e.getMessage()));
            }
    ...
    public void run()
    {
        ...
        while (true)
        {
            try
            {
                ...
                {
                    Station discovMachine = (Station) obj;
                    mr = Msg.prepareMachineReport(discovMachine, Cfg.team);

                    // Senden des Maschinenreports über den privaten Kanal.
                    if (mr != null)
                    {
                        ProtobufMessage machineReport = new ProtobufMessage(2000, 61, mr);
                        peerPrivate.enqueue(machineReport); //Die Nachricht wird über den privaten Peer gesendet.
                        rBLog.debug("Machine Report sent");
                    }
                } else if (obj instanceof MachineInfo)  //wird wahrscheinlich nicht gebraucht (mit Tim abklären)
                {
                    mi = (MachineInfo) obj;
