...
public class ReportMachine
{
    ...
    private static String MACHINE_ZONE;
    private static String MACHINE_ROTATION;
    ...
    public static void main(String[] args)
    {
        ...
        if (args.length < 4)
        {
            System.out.println("Usage: ReportMachine.jar <machine-name> <machine-> <team-name> <encryption-key> [-h <host> (only for a remote refbox)]");
            System.exit(1);
        }
                ...
                try
                {
                    ...
                    if (crypto_setup)
                    {
                        if (bs.getTeamName().equals("LLSF") && bs.getPeerName().equals("RefBox"))
                        {
                            ...
                            MachineReportProtos.MachineReportEntry mi = MachineReportProtos.MachineReportEntry.newBuilder().
                                setName(MACHINE_NAME).
                                //setZone(MACHINE_ZONE).
                                //setRotation(MACHINE_ROTATION).
                                build();
